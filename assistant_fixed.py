...
class EnhancedAssistant:
    ...
    def answer(self, prompt, desktop_image=None, camera_image=None, mode="desktop"):
        ...
        # For "both" mode, use the currently active mode
        if mode == "both":
            mode = active_mode
            print(f"📊 Both mode: Using {mode} as active mode")
...
def audio_callback(recognizer, audio):
    ...
    try:
        ...
        # Always try to get both images for "both" mode
        if current_mode == "both" or current_mode == "desktop":
        ...
        if current_mode == "both" or current_mode == "camera":
        ...
        if current_mode == "both":
            # Use the active mode determined by the monitor
            if active_mode == "desktop" and desktop_image:
                assistant.answer(prompt, desktop_image, camera_image, "both")
            elif active_mode == "camera" and camera_image:
                assistant.answer(prompt, desktop_image, camera_image, "both")
            else:
                print("⚠️ No image sources available in both mode")
        else:
            # Single mode operation
            if (current_mode == "desktop" and desktop_image) or (current_mode == "camera" and camera_image):
...
def display_status():
    ...
    print("\n" + "="*60)
    print("📊 AI VOICE ASSISTANT STATUS - ENHANCED")
    print("="*60)
    print(f"Current Mode: {current_mode.upper()}")
    
    if current_mode == "both":
        print(f"Active Mode: {active_mode.upper()}")
        print("\n🔄 AUTO-DETECTION MODE (ENHANCED):")
    ...
    if current_mode == "both":
        ...
        print("  - Automatic camera reconnection when available")
...
def main():
    ...
    print("🤖 AI Voice Assistant - Enhanced Multi-Mode (FIXED)")
    print("=" * 60)
    print("Features: Desktop Screen Capture + Camera Feed + Voice Recognition")
    print("🔧 ENHANCED: Fixed Option 3 auto-detection with continuous monitoring")
    print()
    ...
    print("3. Both (desktop + camera) - ENHANCED AUTO-DETECTION")
    ...
    if current_mode in ["camera", "both"]:
        ...
        try:
            ...
            if camera_recorder.is_available():
                ...
                if current_mode == "both":
                    print("⚠️ Continuing in desktop-only mode initially")
    ...
    if current_mode == "both":
        print("- System automatically detects and switches between modes")
        print("- Automatic camera reconnection when available")
        print("- Continuous monitoring every 2 seconds")
