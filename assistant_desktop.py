...
class Assistant:
    ...
    def answer(self, prompt, image):
        ...
        response = self.chain.invoke(
            ...
            config={"configurable": {"session_id": "desktop_session"}},
        ).strip()
    ...
    def _create_inference_chain(self, model):
        SYSTEM_PROMPT = """
        You are a witty assistant that will use the chat history and the desktop screen image 
        ...
        You can see the user's desktop screen in real-time, so provide context-aware assistance
        based on what's currently displayed on their screen.
        """
...
# ==== Init ====
print("🖥️ Starting AI Voice Assistant - Desktop Mode")
print("This mode captures and analyzes your desktop screen in real-time.")
print("Press 'q' or ESC to quit, or Ctrl+C to stop.")

model = ChatOpenAI(model="gpt-4o")
assistant = Assistant(model)
recorder = DesktopRecorder().start()
...
try:
    while running:
        ...
        if frame is not None:
            ...
            cv2.imshow("Desktop AI Assistant", preview)
